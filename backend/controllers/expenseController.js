...
//Get all Expense sources // Fixed comment
...
//Delete expense source // Fixed "Delet" to "Delete"
exports.deleteExpense = async (req, res) => {
    try {
        await Expense.findByIdAndDelete(req.params.id);
        res.json({ message: "Expense deleted successfully" }); // Removed extra space
    } catch (error) {
...
exports.downloadExpenseExcel = async (req, res) => {
    ...
    try {
        const expense = await Expense.find({ userId }).sort({ date: -1 }); // Fixed "userid" to "userId"
